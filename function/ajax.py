...
def ajax_payload_explain_func(payload_name):
    try:
        payload = Payload.objects.get(name=payload_name)
        explain = payload.explain                                              #根据name查询payload的explain
        parameter = payload.parameter                                          #查询是否需要额外自定义参数
        return ['200',explain,parameter]
    except:
...
def ajax_add_project_func(username,name,describe,payload,send_email,
    ...
    try:
        Project.objects.create(project_id=project_id,name=name,owner=username,
                               ...
                               diy_payload = diy_payload,payload_parameter=parameter)
        return ['200','True']
    except:
        return ['500','添加失败，请检查字段内容']
